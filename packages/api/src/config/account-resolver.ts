...
export type BuiltinAccountClient = 'anthropic' | 'openai' | 'google' | 'dare' | 'trae' | 'opencode';
...
const BUILTIN_ACCOUNT_MAP: Record<string, { client: BuiltinAccountClient; protocol: AccountProtocol }> = {
  ...
  trae: { client: 'trae', protocol: 'openai' },
  builtin_trae: { client: 'trae', protocol: 'openai' },
...
function normalizeProtocol(clientOrProtocol: string): AccountProtocol {
  ...
  if (clientOrProtocol === 'trae') return 'openai';
...
function accountToRuntimeProfile(ref: string, account: AccountConfig): RuntimeProviderProfile {
  ...
  return {
    ...
    ...(isBuiltin ? { client: BUILTIN_ACCOUNT_MAP[ref]?.client ?? protocolToClient(account.protocol) } : {}),
...
export function validateRuntimeProviderBinding(
  ...
  if (provider === 'trae' && profile.kind !== 'builtin') {
    return 'client "trae" only supports builtin Trae auth';
  }
