...
describe('provider profiles routes', () => {
  ...
  it('GET /api/provider-profiles preserves Trae builtin client identity', async () => {
    const Fastify = (await import('fastify')).default;
    const { providerProfilesRoutes } = await import('../dist/routes/provider-profiles.js');
    const app = Fastify();
    await app.register(providerProfilesRoutes);
    ...
    const projectDir = await makeTmpDir('trae-list');
    setGlobalRoot(projectDir);
    try {
      await mkdir(join(projectDir, '.cat-cafe'), { recursive: true });
      await writeFile(
      ...
      const trae = body.providers.find((profile) => profile.id === 'trae');
      assert.ok(trae, 'Trae builtin account should be listed');
      assert.equal(trae.client, 'trae');
      assert.equal(trae.displayName, 'Trae (client-auth)');
    } finally {
      restoreGlobalRoot();
      await rm(projectDir, { recursive: true, force: true });
      await app.close();
